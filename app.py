...
# ─── Config ───────────────────────────────────────────────────────────────────
st.set_page_config(
    page_title="⚽ Voetbal Aanwezigheid",
    page_icon="⚽",
    layout="wide",
)
...
# ─── Statistieken bovenaan (5 kolommen) ──────────────────────────────────────
...
if echte_wedstrijden and data["spelers"]:
    ...
    totaal_afwezig  = sum(len(s.get("afwezig",  [])) for s in echte_wedstrijden.values())
    totaal_blessure = sum(len(s.get("blessure", [])) for s in echte_wedstrijden.values())
    gem_blessure    = round(totaal_blessure / totaal_wedstrijden, 1)
else:
    gem_aanwezigheid = gem_blessure = 0

beste = "–"
if echte_wedstrijden and data["spelers"]:
    stats_dict = {naam: speler_stats(naam) for naam in speler_namen}
    beste = max(stats_dict, key=lambda n: stats_dict[n][0])
...
with c5:
    st.markdown(f"""<div class="stat-card">
        ...
        <div class="stat-label" style="color:#00c853;font-size:1rem;font-weight:600">{beste}</div>
        <div class="stat-label">Meest aanwezig</div>
    </div>""", unsafe_allow_html=True)
